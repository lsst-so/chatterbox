...
from .style import PROJECTION, add_galactic_plane, localization_levels, use_headless_backend
...
def plot_template_coverage(
    ...
    """Draw one panel per band: template visit counts with the localization.

    Parameters
    ----------
    coverage : `TemplateCoverage`
        Per-band visit-count maps.
    localization : `Localization`
    ...
    vmax : `int`, optional
        Upper limit of the visit-count colour scale. Defaults to
        ``max(2, min_visits + 1)`` so the "has a template" boundary is visible.
    dpi : `int`
        Output resolution.
    skip_empty : `bool`
    ...
    if vmax is None:
        vmax = max(2, coverage.min_visits + 1)
    ...
    for n, band in enumerate(drawn, start=1):
        ...
        nvis = np.array(coverage.maps[band], dtype=float)
        display = np.where(nvis > 0, nvis, np.nan)
        img = ax.imshow_hpx(display, nested=False, cmap="Reds", vmin=0, vmax=vmax)
        if levels:
            ax.contour_hpx(
        ...
        ax.set_title(
            f"{band} band: {coverage.area_deg2(band):,.0f} deg$^2$ with >= {coverage.min_visits} visit(s)\n"
    ...
    if img is not None:
        ...
        # Visit counts are integers; the default float ticks read oddly.
        cbar.set_ticks(np.arange(0, vmax + 1))
        cbar.set_ticklabels([f"{n:d}" if n < vmax else f"{int(vmax)}+" for n in range(int(vmax) + 1)])
        cbar.set_label("Existing template visits per pixel")

    stamp = f" (built {coverage.built_at})" if coverage.built_at else ""
    fig.suptitle(f"Existing LSST template coverage vs localization{stamp}", fontsize=11)
